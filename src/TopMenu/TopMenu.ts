...
function targetClick(allTargets: NodeListOf<HTMLElement>, ev: MouseEvent) {
  ...
  /* console.log('[TopMenu:targetClick]', {
   *   allTargets,
   *   dataset,
   *   targetNode,
   *   parent,
...
export function initTopMenu() {
  ...
  if (menuNode) {
    ...
    const targetClickBound = targetClick.bind(null, targets);
    /* console.log('[TopMenu:initTopMenu] SubMenu cloned', {
     *   targets,
     *   menuNodeCopy,
     *   menuNode,
    ...
    targets.forEach((node) => {
      node.addEventListener('click', targetClickBound);
